// $Id: type_tensor.h 3874 2010-07-02 21:57:26Z roystgnr $
...
//we already included it  by vector_value.h
//so there is no need -osman
...
public:
 ...
 // void print(std::ostream& os = out) const;
  ...
  friend std::ostream& operator << (std::ostream& os, const TypeTensor<T>& t)
  {
    //t.print(os);
    //osman
     os<<"("<<t._coords[0]<<", "<<t._coords[1]<<", "<<t._coords[2]<<",\n "
			<<t._coords[3]<<", "<<t._coords[4]<<", "<<t._coords[5]<<",\n "
			<<t._coords[6]<<", "<<t._coords[7]<<", "<<t._coords[8]<<")";
    return os;
  }
